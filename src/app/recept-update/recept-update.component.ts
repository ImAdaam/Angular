...
export class ReceptUpdateComponent {
  recipe: Recept = { id: 0, cim: '', leiras: '', hozzavalok: [], allergenek: [], user: { id: 0, username: '', email: '', password: '' } };
  ...
  ngOnInit(): void {
    ...
    this.recipe = this.inMemoryDataService.getRecipes().find(r => r.id === recipeId) || this.recipe;
  }

  updateRecipe(): void {
    this.inMemoryDataService.updateRecipe(this.recipe);
...
export class ReceptUpdateComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.currentRecipe) {
      ...
      this.currentRecipe.borito = this.recipeForm.value.coverImage;
